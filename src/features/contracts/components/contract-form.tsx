  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 rounded border border-slate-200 bg-slate-50 p-4">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <input className="w-full rounded border border-slate-300 px-3 py-2 text-sm" placeholder="Titre du contrat" {...register('title')} />
        <label className="flex w-full items-center gap-2 rounded border border-slate-300 bg-white px-3 py-2 text-sm text-slate-700">
          <input type="checkbox" {...register('autoRenewal')} />
          Auto-renouvellement
        </label>

        <input type="date" className="w-full rounded border border-slate-300 px-3 py-2 text-sm" {...register('startDate')} />
        <input type="date" className="w-full rounded border border-slate-300 px-3 py-2 text-sm" {...register('endDate')} />
      </div>

      {Object.keys(errors).length ? <p className="text-xs text-rose-600">Merci de vérifier les champs contrat.</p> : null}

      <button disabled={isPending} className="w-full rounded bg-slate-900 px-3 py-2 text-sm text-white disabled:opacity-60 sm:w-auto">
