...
export function ClientContractsTab({ client, contracts, templates }: { client: ClientWithRelations; contracts: Contract[]; templates: ContractTemplate[] }) {
  const [editingId, setEditingId] = useState<string | null>(null);

  return (
    <div className="space-y-4">
      <ContractGenerator client={client} templates={templates} />
      <ContractForm clientId={client.id} onDone={() => setEditingId(null)} />
            ...
            <div key={contract.id} className="space-y-2 rounded border border-slate-200 bg-white p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div>
                  ...
                  {contract.generatedContent ? <p className="mt-1 text-xs text-emerald-700">Généré depuis un modèle</p> : null}
                </div>
              ...
              <dl className="grid gap-x-4 gap-y-2 rounded border border-slate-100 bg-slate-50 p-3 text-xs text-slate-700 sm:grid-cols-2">
                ...
                {contract.generatedContent ? (
                  <div className="sm:col-span-2">
                    <dt className="font-medium text-slate-900">Aperçu du contrat généré</dt>
                    <dd className="mt-1 max-h-72 overflow-auto whitespace-pre-wrap rounded border border-slate-200 bg-white p-3 font-mono text-[11px] leading-5">
                      {contract.generatedContent}
                    </dd>
                  </div>
                ) : null}
              ...
              {editingId === contract.id ? <ContractForm clientId={client.id} contract={contract} onDone={() => setEditingId(null)} /> : null}
              <ContractUploadField clientId={client.id} contract={contract} />
            </div>
          ))}
        </div>
      )}
